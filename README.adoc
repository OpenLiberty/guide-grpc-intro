...
The endpoint creates a [hotspot=createClient2 file=2]`SystemServiceClient` and uses the client to collect system properties that start with `os.`, shuts down the channel, and displays the values from the `system` service response.
...
The endpoint creates a [hotspot=createClient3 file=2]`SystemServiceClient` and uses the client to collect system properties that start with `user.`, shuts down the channel, and displays the values from the `system` service response.
...
The endpoint creates a [hotspot=createClient4 file=2]`SystemServiceClient` and uses the client to collect system properties that start with `java.`, shuts down the channel, and displays the values from the `system` service response.
