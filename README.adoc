...
After a connection is created between the two services, call the [hotspot=defineClient file=1]`client.getPropertiesClient()` API to get a stream, collect the properties started with [hotspot=collectUserProperties file=1]`user.`, create and send [hotspot=clientStream file=1]`SystemPropertyName` messages individually to the server. When all property names are sent, finish the `stream` by calling the [hotspot=clientCompleted file=1]`onCompleted()`;
