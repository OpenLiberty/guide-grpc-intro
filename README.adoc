...
Each test case tests one type of the gRPC servers that you implemented by making calls with the client through a real stub using the in-process channel.

// Create SystemTest.java
[role="code_command hotspot file=0", subs="quotes"]
----
...
[source, Java, linenums, indent=0, role="code_column hide_tags=copyright"]
----
include::finish/system/src/test/java/it/io/openliberty/guides/system/SystemTest.java[]
----
...
The [hotspot=testGetProperty file=0]`testGetProperty()` tests the [hotspot=getProperty file=1]`getProperty()` unary RPC call and confirms that a response is received. 

The [hotspot=testGetServerStreamingProperties file=0]`testGetServerStreamingProperties()` tests the [hotspot=getServerStreamingProperties file=1]`getServerStreamingProperties()` server streaming RPC call and confirms that a response is received. 

The [hotspot=testGetClientStreamingProperties file=0]`testGetClientStreamingProperties()` tests the [hotspot=getClientStreamingProperties file=1]`getClientStreamingProperties()` client streaming RPC call and confirms that a response is received. 

The [hotspot=testGetBidirectionalProperties file=0]`testGetBidirectionalProperties()` tests the [hotspot=getBidirectionalProperties file=1]`getBidirectionalProperties()` bidirectional streaming RPC call and confirms that a response is received. 
...
//  T E S T S
// -------------------------------------------------------
// Running it.io.openliberty.guides.system.SystemIT
// ...
// Tests run: 4, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 0.527 s - in it.io.openliberty.guides.system.SystemIT
...
=== Creating unit tests for gRPC client


=== Creating integration tests

// update the first paragraph to make clear, here is doing integration test using RESTful APIs

Each test case tests one type of the gRPC calls that you implemented via the RESTful APIs.
