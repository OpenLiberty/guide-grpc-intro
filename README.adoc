...
=== Creating unit tests for gRPC server

// update the first paragraph to make clear, here is doing unit test for gRPC server and describe the 2 dependencies grpc-testing and mockito-core
...
Unit testing is a critical part of any software development process. With gRPC, you can easily create unit tests for your server application. In this section, you can learn how to create unit tests for your gRPC server by using JUnit and Mockito.

The [hotspot file=0]`pom.xml` file lists the [hotspot=grpc-testing file=0]`grpc-testing` and [hotspot=mockito-core file=0]`mockito-core` dependencies.

The [hotspot=grpc-testing file=0]`grpc-testing` dependency provides utilities for testing gRPC services and is used for creating a mock gRPC server for simulating client-server communication during testing. The [hotspot=mockito-core file=0]`mockito-core` dependency is needed to enable the use of Mockito.

// file 1
// Create SystemServiceTest.java
[role="code_command hotspot file=1", subs="quotes"]
----
#Create the `SystemTest` class.#
...
The [hotspot=inProcessChannel file=1]`inProcessChannel` creates a direct in-process channel that connects to the server running in the same JVM process as the test. This means that the unit test is able to make calls to the gRPC server by using the same method signatures and functionality as the gRPC client, even though they are using different channels to create their respective stubs.

The [hotspot=testGetProperty file=1]`testGetProperty()` tests the unary RPC call to retrieve a single system property value.

The [hotspot=testGetServerStreamingProperties file=1]`testGetServerStreamingProperties()` tests the server streaming RPC call to retrieve multiple system property values with a given prefix.

The [hotspot=testGetClientStreamingProperties file=1]`testGetClientStreamingProperties()` tests the client streaming RPC call to retrieve multiple system property values with given names.

The [hotspot=testGetBidirectionalProperties file=1]`testGetBidirectionalProperties()` tests the bidirectional streaming RPC call to retrieve multiple system property values with given names.
...
-------------------------------------------------------
 T E S T S
-------------------------------------------------------
Running it.io.openliberty.guides.system.SystemServiceTest

Tests run: 4, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 0.527 s - in it.io.openliberty.guides.system.SystemServiceTest
...
In this section, you'll write integration tests using RESTful APIs to test your gRPC application.
