...
The [hotspot=getProperty file=0]`getProperty()` method implements the rpc unary call defined in the proto file. Use the [hotspot=pName file=0]`getPropertyName()` getter method that is automatically generated by gRPC to retrieve the property name from the client and store into the `pName` variable. Store the System property value into the [hotspot=pValue file=0]`pValue` variable. Create a [hotspot=response file=0]`SystemPropertyValue` message type defined in the proto file by using the gRPC APIs. Then, send the message to the client service by using [hotspot=next file=0]`onNext()` and [hotspot=complete file=0]`onComplete()` APIs.
