...
The [hotspot=testGetProperty file=1]`testGetProperty()` tests the [hotspot=getProperty file=2]`getProperty()` unary RPC call and confirms that a response is received. 

The [hotspot=testGetServerStreamingProperties file=1]`testGetServerStreamingProperties()` tests the [hotspot=getServerStreamingProperties file=2]`getServerStreamingProperties()` server streaming RPC call and confirms that a response is received. 

The [hotspot=testGetClientStreamingProperties file=1]`testGetClientStreamingProperties()` tests the [hotspot=getClientStreamingProperties file=2]`getClientStreamingProperties()` client streaming RPC call and confirms that a response is received. 

The [hotspot=testGetBidirectionalProperties file=1]`testGetBidirectionalProperties()` tests the [hotspot=getBidirectionalProperties file=2]`getBidirectionalProperties()` bidirectional streaming RPC call and confirms that a response is received. 

=== Running the tests

Because you started Open Liberty in dev mode, you can run the tests by pressing the `enter/return` key from the command-line session where you started dev mode.

Press the `enter/return` key from the command-line session where you started the query service. You will see the following output:
...
Next, press the `enter/return` key from the command-line session where you started the system service. You will see the following output:
