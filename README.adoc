...
In the [hotspot=tearDown file=1]`tearDown()` static method, shutdown the `inProcessChannel` in-process channel and then the `inProcessServer` in-process gRPC server.
...
=== Creating integration tests for the query service
