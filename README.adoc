...
The [hotspot=getProperty file=0]`getProperty()` is the implementation of the rpc call defined in the proto file. The [hotspot=pName file=0]`pName` variable stores the request string, notice the use of the getter `getPropertyName`, this is also automatically generated by gRPC. The [hotspot=pValue file=0]`pValue` gets and stores the System property value. The [hotspot=response file=0]`value` variable stores the built response to be sent back. Notice the variable type `SystemPropertyValue`, this is the message type that was defined in the proto file. The built response is then sent to the query service by using [hotspot=next file=0]`onNext()` and [hotspot=complete file=0]`onComplete()`.
