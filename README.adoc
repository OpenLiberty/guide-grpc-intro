...
----
The `PropertiesResource` class provides RESTfull endpoints to interact with the `system` service. The [hotspot=unary file=0]`/query/properties/${propertyName}` endpoint is a unary service call to get the property value from the `system` service. The endpoint creates a channel, use the channel to create a client by the `SystemServiceGrpc.newBlockingStub()` API, use the client to get the property value, and then shutdown the channel.
