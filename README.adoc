...
Let's first start the query service in dev mode by using the following command. 
...
mvn -pl query liberty:dev
```
The PropertiesResource class acts like a client calling the query service for a response. 
...
The system service isreceives user properties  to it in a .
