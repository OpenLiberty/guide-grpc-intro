...
The [hotspot=getProperty file=0]`getProperty()` method implements the rpc unary call defined in the proto file. Use the [hotspot=pName file=0]`getPropertyName()` getter method that is automatically generated by gRPC to retrieve the property name from the client, and store into the `pName` variable. Get and store the System property value into the [hotspot=pValue file=0]`pValue` variable. Use the the gRPC APIs to create a [hotspot=response file=0]`SystemPropertyValue` message that its type is defined in the `SystemService.proto` file. Then, send the message to the client service by using [hotspot=next file=0]`onNext()` and [hotspot=complete file=0]`onComplete()` APIs.
...
Add the [hotspot=grpc file=1]`grpc-1.0` feature to the Liberty server configuration file. The feature enables gRPC compatibility with the Liberty server.
...
----
The `PropertiesResource` class provides RESTful endpoints to interact with the `system` service. The [hotspot=unary file=2]`/query/properties/${propertyName}` endpoint uses a unary service call to get the property value from the `system` service. The endpoint creates a channel, uses the channel to create a client by the `SystemServiceGrpc.newBlockingStub()` API, uses the client to get the property value, shutdowns the channel, and then returns the value that the `system` service responses immediately.
...
Add the [hotspot=grpc file=3]`grpc-1.0` and [hotspot=grpcClient file=3]`grpcClient-1.0` features to the Liberty server configuration file. Both features enable gRPC compatibility with the Liberty server.

Point your browser to the http://localhost:9081/query/properties/os.name URL to test out the unary service. You will see your operating system name. You can try out the URL with other property name.
