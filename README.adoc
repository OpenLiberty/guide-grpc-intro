...
----
include::finish/system/src/main/java/io/openliberty/guides/system/SystemProto.java[]
----
...
== Implementing server streaming 
The next service we'll implement is `Server Streaming`. The query service will send a request using the `os` endpoint and the system service will stream the os properties back to the query service. 

// Commands for system service

Update the SystemService class to include the implementation of `getOSProperties()`
