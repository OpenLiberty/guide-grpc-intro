...
The [hotspot=getPropertiesServer file=0]`getPropertiesServer()` method implements server streaming rpc call. Use the [hotspot=prefix file=0]`getPropertiesWithPrefix()` getter method to retrieve the property prefix from the client. Filter out the properties that starts with the [hotspot=filter file=0]`prefix`. For each property, build a [hotspot=serverMessage file=0]`SystemProperty` message and stream the message to the client through the `StreamObserver` by using its [hotspot=serverNext1 file=0]`onNext()` method. When all properties are streamed, finish the streaming by calling the [hotspot=serverComplete file=0]`onComplete()` method.
