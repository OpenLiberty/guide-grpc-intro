...
Next, implement the client code that makes the unary RPC calls to the server in the `query` service.

// Create SystemServiceClient
[role="code_command hotspot file=2", subs="quotes"]
----
#Create the `SystemServiceClient` class.#
`query/src/main/java/io/openliberty/guides/query/client/SystemServiceClient.java`
----
...
----
include::finish/query/src/main/java/io/openliberty/guides/query/client/SystemServiceClient.java[]
----

The `SystemServiceClient` class provides client code that interact with the `system` service server. 

The [hotspot=constructor file=2]`SystemServiceClient` constructor takes in a `hostname` and `port` as arguments and uses them to create a `ManagedChannel` with plaintext communication. This channel is used to initialize both a blocking and asynchronous stub for the `system` service. For this sample application, having both stubs allows the client to support different types of calls, such as unary and different streaming calls, providing more flexibility and options for the client's interaction with the server.

The [hotspot=shutdown file=2]`shutdown()` method simply shuts down the gRPC channel that is created in the constructor.

The [hotspot=unary file=2]`getProperty()` method uses the unary service call to retrieve the property value from the `system` service. It calls the [hotspot=getProperty file=2]`SystemService.getProperty()` method on the `blockingStub` client object and returns the response value from the `system` service.
...
The `PropertiesResource` class provides RESTful endpoints to use the [hotspot=createClient1 file=3]`SystemServiceClient` class to interact with the `system` service and return the results. The [hotspot=unary file=3]`/query/properties/${property}` endpoint creates a [hotspot=createClient1 file=3]`SystemServiceClient` client object using the injected `SYSTEM_HOST` and `SYSTEM_PORT` values. The endpoint then uses the client to get the property value, shuts down the client, and immediately returns the value from the `system` service response.
...
Add the [hotspot=grpc file=4]`grpc` and [hotspot=grpcClient file=4]`grpcClient` features to the Liberty server configuration file for the `query` service. These two features enable gRPC server and client support on Liberty respectively. Configure the [hotspot=grpcClientConfig file=4]`grpcClient` feature with the `headersToPropagate` and `target` attributes. If you want to learn more about `grpcClient` feature configuration, see the https://openliberty.io/docs/latest/reference/config/grpcClient.html[GRPC Client Properties^].
...
In the server streaming call, the `query` client service provides the `getServerStreamingProperties()` method, which requests the property names that start with `os.` from the `system` server service. The `system` service streams the property values back to the `query` client.
...
Update the `SystemServiceClient` class to implement the `getServerStreamingProperties()` method.

[role="code_command hotspot file=1", subs="quotes"]
----
#Replace the `SystemServiceClient` class.#
`query/src/main/java/io/openliberty/guides/query/client/SystemServiceClient.java`
----
...
This [hotspot=serverStreaming file=1]`getServerStreamingProperties()` method calls the [hotspot=getServerStreamingProperties file=1]`getServerStreamingProperties()` method of the `asyncStub` client object with an implementation of the `StreamObserver` interface. The [hotspot=onNext1 file=1]`onNext()` method receives messages streaming from the server service individually and stores them into the [hotspot=placeholder hotspot=storeInPlaceholder file=1]`properties` placeholder. After all properties are received, the `system` service returns the placeholder. Because the RPC call is asynchronous, a [hotspot=countDownLatch1 hotspot=countDownLatch2 hotspot=countDownLatch3 file=1]`CountDownLatch` instance synchronizes the streaming flow.

Then, update the `PropertiesResource` class to implement the `/query/properties/os` endpoint of the `query` service.
...
The endpoint creates a [hotspot=createClient2 file=2]`SystemServiceClient` client object, uses the client to collect system [hotspot=osProperties file=2]`os.` properties, shuts down the client, and returns the values from the `system` service response.
...
In the client streaming call, the `query` client service provides the `getClientStreamingProperties()` method, which streams the user properties to the `system` server service. The `system` service returns a map of user properties with their values.
...
Update the `SystemServiceClient` class to implement the `getClientStreamingProperties()` method.
...
The [hotspot=clientStreaming file=1]`getClientStreamingProperties()` method calls the [hotspot=getClientStreamingProperties file=1]`getClientStreamingProperties()` method of the `asyncStub` client object to get a `stream` and collect the properties with property names that are prefixed by [hotspot=clientPropertyPrefix hotspot=clientCollectProperties file=1]`propertyPrefix` argument. It then creates a [hotspot=clientMessage1 file=1]`SystemPropertyName` message individually and sends the message to the server by the [hotspot=streamOnNext1 file=1]`stream::onNext` action. When all property names are sent, the [hotspot=clientCompleted1 file=1]`onCompleted()` method is called to finish the streaming. Again, a [hotspot=countDownLatch4 hotspot=countDownLatch5 hotspot=countDownLatch6 file=1]`CountDownLatch` instance synchronizes the streaming flow.

// Replace PropertiesResource
Then, update the `PropertiesResource` class to implement the `/query/properties/user` endpoint of the `query` service.
...
The endpoint creates a [hotspot=createClient3 file=2]`SystemServiceClient` client object, uses the client to collect system [hotspot=userProperties file=2]`user.` properties, shuts down the client, and returns the values from the `system` service response.
...
In the bidirectional streaming call, the `query` client service provides the `getBidirectionalProperties()` method, which streams the property names that start with `java.` to the `system` server service. The `system` service streams the property values back to the `query` client.
...
Update the `SystemServiceClient` class to implement the `getBidirectionalProperties()` method.

// Replace SystemServiceClient
[role="code_command hotspot file=1", subs="quotes"]
----
#Replace the `SystemServiceClient` class.#
`query/src/main/java/io/openliberty/guides/query/client/SystemServiceClient.java`
...
The [hotspot=bidirectionalStreaming file=1]`getBidirectionalProperties()` method calls the [hotspot=getBidirectionalProperties file=1]`getBidirectionalProperties()` method of the `asyncStub` client object with an implementation of the `StreamObserver` interface. The [hotspot=onNext2 file=1]`onNext()` method receives messages that are streaming from the server individually and stores them into the `properties` placeholder. Then, collect the properties. For each property name that starts with [hotspot=bidiPropertyPrefix hotspot=bidiCollectProperties file=1]`propertyPrefix` argument, a [hotspot=clientMessage2 file=1]`SystemPropertyName` message is created and sent to the server by the [hotspot=streamOnNext2 file=1]`stream::onNext` action. When all property names are sent, the streaming is ended by calling the [hotspot=clientCompleted2 file=1]`onCompleted()` method. Again, a [hotspot=countDownLatch7 hotspot=countDownLatch8 hotspot=countDownLatch9 file=1]`CountDownLatch` instance synchronizes the streaming flow.
...
The endpoint creates a [hotspot=createClient4 file=2]`SystemServiceClient` client object, uses the client to collect system [hotspot=javaProperties file=2]`java.` properties, shuts down the client, and returns the values from the `system` service response.
...
You can test your application manually, but automated tests ensure code quality because they trigger a failure whenever a code change introduces a defect. In this section, you'll create unit tests for the gRPC server service and integration tests for the `query` service.
...
The required dependencies are already added to the `pom.xml` Maven configuration file for you, including JUnit5, [hotspot=grpc-testing file=0]`grpc-testing`, and [hotspot=mockito-core file=0]`mockito-core` libraries. The `grpc-testing` dependency provides utilities for testing gRPC services and is used for creating a mock gRPC server for simulating client-server communication during testing. The `mockito-core` dependency is needed to enable the use of Mockito.
...
`system/src/test/java/it/io/openliberty/guides/system/SystemTest.java`
----
...
In the [hotspot=tearDown file=1]`tearDown()` static method, shutdown the `inProcessChannel` in-process channel and the `inProcessServer` in-process gRPC server.
