...
The endpoint creates a [hotspot=createChannel file=1]`channel` to the `system` service and a [hotspot=createClient file=1]`client` by using the `SystemServiceGrpc.newStub()` API. Then, call the [hotspot=getPropertiesServer file=1]`getPropertiesServer()` method with an implementation of the `StreamObserver` interface. The [hotspot=onNext file=1]`onNext()` method receives messages streaming from the server individually. After all properties are received, shutdown the [hotspot=closeConnection file=1]`channel`.  Because the rpc call is asynchronous, use the [hotspot=countDownLatch1 hotspot=countDownLatch2 hotspot=countDownLatch3 file=1]`CountDownLatch` to synchronize the streaming flow.
