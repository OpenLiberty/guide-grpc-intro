...
The endpoint creates a [hotspot=createClient4 file=2]`SystemServiceClient` and uses the client to collect system properties that start with `java.`, shuts down the channel, and returns the values from the `system` service response.
