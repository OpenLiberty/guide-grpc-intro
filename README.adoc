...
Add the [hotspot=grpc file=1]`grpc-1.0` feature to the Liberty server configuration file. That feature enables gRPC server compatibility with the Liberty server.
...
Add the [hotspot=grpc file=3]`grpc-1.0` and [hotspot=grpcClient file=3]`grpcClient-1.0` features to the Liberty server configuration file. Both features enable gRPC server and client compatibility with the Liberty server.
