...
The endpoint creates a [hotspot=createClient2 file=3]`SystemServiceClient` using the injected `SYSTEM_HOST` and `SYSTEM_PORT` values. It then uses the client to collect system properties that start with `os.`, shuts down the channel, and displays the values from the `system` service response.
