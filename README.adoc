...
:page-seo-title: Exchanging system properties between client and a server by using gRPC calls
...
You will learn how to create various gRPC services by using protocol buffers and how to implement them with Open Liberty. You will use Maven throughout the guide to generate the gRPC stubs and deploy the microservice as well as to interact with the running Liberty instance. 
...
Add [hotspot=grpc file=1]`grpc()` and [hotspot=grpc-stub file=1]`grpc-stub()` dependencies to the pom files. The [hotspot=protobufmavenplugin file=1]`os-maven-plugin()` automatically install the correct version of Protobuf compiler. 

Run the command below to generate the abstract classes. 
...
Let's start by implementing the first service, the unary call. In this service the query service will send one property to the system service which will return the property value. This type of service resembles the traditional REST API. 
...
----
The `PropertiesResource` class provides endpoints to interact with the system service. The `query/<propertiesName>` endpoint makes a unary request to return the property value from system service.
...
Now open another terminal window and start the system service in dev mode by using the following command. 
...
The SystemService class implements the `SystemServiceGrpc` class generated by the proto file, in this class we will implement the four types of services we defined in the proto file. 
...
The [hotspot=getProperty file=0]`getProperty()` is the implementation of the rpc call defined in the proto file. The [hotspot=pName file=0]`pName()` variable stores the request string, notice the use of the getter `getPropertyName`, this is also automatically generated using the mesasge type. The [hotspot=pValue file=0]`pValue()` gets and stores the System property value. The [hotspot=response file=0]`value()` variable stores the built response to be sent back. Notice the variable type `SystemPropertyValue`, this is the message type we defined in the proto file. The built response is then sent to the query service using [hotspot=complete file=0]`onNext()` and [hotspot=complete file=0]`onComplete()`

Open another terminal window and run the following curl command to test out the unary service. Replace `<propertiesName>` with the name of the property you would like to see, an example is `os.name`. You should see your system os name in the terminal.
...
== Implementing server streaming 
The next service we'll implement is `Server Streaming`. The query service will send a request using the `os` endpoint and the system service will stream the os properties back to the query service. When streaming, there is a channel created between the query and the system service which is only closed by the system service after it has sent the last message to the query service. 
...
First a [hotspot=createChannel file=0]`channel()` is created between query and system services using the stubs generated by the proto file. Then the properties are [hotspot=sendProperties file=0]`streamed()` individually from the system service. After all of the properties are received, the [hotspot=closeConnection file=0]`channel()` is closed. 
...
The [hotspot=getPropertiesServer file=0]`getPropertiesServer()` method implements server streaming for the system service. The properties are [hotspot=serverStreaming file=0]`individually streamed()` to the query service. The message `server streaming was completed!` is printed in the system service terminal at the conclusion of the method. 
...
The next service we'll implement is `Client Streaming`. The query service will stream user properties to the system service using the `user` endpoint and the system service will return a map of those user properties.
// Commands for query service
// Replace PropertiesResource
...
After a connection is created between the two services, a [hotspot=defineClient file=0]`client[]` is defined to ensure the expected behaviour. The [hotspot=getPropertiesClientStreaming file=0]`user properties[]` are [hotspot=clientStream file=0]`streamed[]` to the system service. 
...
The [hotspot=getPropertiesClient file=0]`getPropertiesClient()[]` method implements client streaming for the system service. In this method, system service [hotspot=receivingProperties file=0]`receives[]` the user properties from the query service and stores them in a [hotspot=clientStreamingMap file=0]`Map[]`, and after the streaming is completed the stored map is [hotspot=clientStreamingCompleted file=0]`returned[]` to the query service. 
...
== Implementing bidirectional streaming 
The next service we'll implement is `Bidirectional Streaming`. The query service will stream java properties while the system service will stream the values of those properties back to the query service. 
// Commands for query service
// Replace PropertiesResource
[role="code_command hotspot file=0", subs="quotes"]
----
...
Similar to the client streaming service, the [hotspot=bidirectionalStreaming file=0]`birectional streaming[]` also receives streamed properties from the query service but in addition it also streams the values for the properties received back to the query service. 
...
The [hotspot=getPropertiesBidirect file=0]`getPropertiesBidirect()[]` method implements bidirectional streaming for the system service. In this method, system service [hotspot=receiveBidirectionalProperties file=0]`receives and sends[]` the java properties with their respective values back to the query service.
...
`finish/query/src/test/java/it/io/openliberty/guides/QueryIT.java`
----

QueryIT.java
[source, Java, linenums, indent=0, role="code_column hide_tags=copyright"]
----
include::finish/query/src/test/java/it/io/openliberty/guides/QueryIT.java[]
----
