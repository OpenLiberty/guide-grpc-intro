...
Start by implementing the first service call, the unary call. In this service call, the `SystemServiceClient` client service sends a property to the `system` server service, which returns the property value. This type of service call resembles a RESTful API. 
...
The [hotspot=constructor file=2]`SystemServiceClient` constructor takes in a `host` and `port` as arguments and uses them to create a `ManagedChannel` with plaintext communication. This channel is used to create a blocking and asynchronous stub for the `system` service.

The [hotspot=shutdown file=2]`shutdown()` method simply shuts down the gRPC channel that was created in the constructor.

The [hotspot=unary file=2]`getPropertiesString()` method uses a unary service call to retrieve the property value from the `system` service. It calls the [hotspot=getProperty file=2]`getProperty()` method on the `blockingStub` client object and returns the response value from the `system` service.
...
The `PropertiesResource` class provides RESTful endpoints to use the [hotspot file=2]`SystemServiceClient` to interact with the `system` service and display the results. The [hotspot=unary file=3]`/query/properties/${property}` endpoint creates a [hotspot=createChannel1 file=3]`SystemServiceClient` using the injected `SYSTEM_HOST` and `SYSTEM_PORT` values. It then uses the client to get the property value, shuts down the channel, and displays the value from the `system` service response.
...
In the server streaming call, the `getOSProperties()` method of the `SystemServiceClient` class initiates a server streaming call to the `system` server service, requesting properties that start with `os.`. The `system` service streams the properties back to the client service over a channel, which is closed by the `system` service only after sending the last message to the `SystemServiceClient` client service. Finally, the `getOSProperties()` method returns the received properties.
...
Update the `SystemServiceClient` class to implement the `getOSProperties()` method.
...
This [hotspot=serverStreaming file=1]`getOSProperties()` method calls the [hotspot=getServerStreamingProperties file=1]`getServerStreamingProperties()` method with an implementation of the `StreamObserver` interface. The [hotspot=onNext1 file=1]`onNext()` method receives messages streaming from the server service individually and stores them into the `properties` placeholder. After all properties are received, the `system` service returns the placeholder. Because the RPC call is asynchronous, a [hotspot=countDownLatch1 hotspot=countDownLatch2 hotspot=countDownLatch3 file=1]`CountDownLatch` instance synchronizes the streaming flow.
...
The endpoint creates a [hotspot=createChannel2 file=3]`SystemServiceClient` using the injected `SYSTEM_HOST` and `SYSTEM_PORT` values. It then uses the client to collect system properties that start with `os.`, shuts down the channel, and displays the values from the `system` service response.
...
// In the client streaming call, the `SystemServiceClient` client service provides the `getUserProperties()` method, which streams the user properties to the `system` server service. The `system` service returns a map of user properties with their values.
...
// Update the `SystemServiceClient` class to implement the `getUserProperties()` method.
...
// The [hotspot=clientStreaming file=1]`getUserProperties()` method calls the [hotspot=getClientStreamingProperties file=1]`getClientStreamingProperties()` method on the [hotspot=clientStreamingAsyncStub file=1]`asyncStub` client to get a `stream` and collect the properties with property names that are prefixed by [hotspot=collectUserProperties file=1]`user.`. It then creates a [hotspot=clientMessage1 file=1]`SystemPropertyName` message individually and sends the message to the server by the [hotspot=streamOnNext1 file=1]`stream::onNext` action. When all property names are sent, the [hotspot=clientCompleted1 file=1]`onCompleted()` method is called to finish the streaming. Again, a [hotspot=countDownLatch4 hotspot=countDownLatch5 hotspot=countDownLatch6 file=1]`CountDownLatch` instance synchronizes the streaming flow.
...
// The endpoint creates a [hotspot=createChannel3 file=2]`SystemServiceClient` using the injected `SYSTEM_HOST` and `SYSTEM_PORT` values. It then uses the client to collect system properties that start with `user.`, shuts down the channel, and displays the values from the `system` service response.
...
// In the bidirectional streaming call, the `SystemServiceClient` client service provides the `getJavaProperties()` method, which streams the property names that start with `java.` to the `system` server service. The `system` service streams the property values back to the `SystemServiceClient` client.
...
// Update the `SystemServiceClient` class to implement the `getJavaProperties()` method.
...
// The endpoint creates a [hotspot=createChannel4 file=2]`SystemServiceClient` using the injected `SYSTEM_HOST` and `SYSTEM_PORT` values. It then uses the client to collect system properties that start with `java.`, shuts down the channel, and displays the values from the `system` service response.
