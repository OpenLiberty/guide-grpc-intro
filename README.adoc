...
The first few lines define the `syntax`[hotspot=basicConfig file=0], `package`[hotspot=basicConfig file=0], and `option`[hotspot=basicConfig file=0] basic configuration of the `.proto` file. The [hotspot=SystemService file=0]`SystemService` service contains the four service calls that will be implemented in the coming sections.

The `getProperty`[hotspot=getProperty file=0] rpc call defines the unary call. In this call, the client side sends a property name message to the server side that returns back a message with the property value. The `SystemPropertyName`[hotspot=SystemPropertyName file=0] and `SystemPropertyValue`[hotspot=SystemPropertyValue file=0] message types define that the propertyName and propertyValue must be string.

The `getPropertiesServer`[hotspot=getPropertiesServer file=0] rpc defines the server streaming call. The client side sends a property name message to the server side. The server returns back a stream of `SystemProperty`[hotspot=SystemProperty file=0] messages. Each `SystemProperty` message contains a `propertyName` and `propertyValue`.

The `getPropertiesClient`[hotspot=getPropertiesClient file=0] rpc defines the client streaming call. The client side streams `SystemPropertyName` properties to the server side. The server returns back a `SystemProperties`[hotspot=SystemProperties file=0] message that contains a map of the properties with their respective values.


The `getPropertiesBidirect`[hotspot=getPropertiesBidirect file=0] rpc defines the bidirectional streaming call. In this service, the client side streams `SystemPropertyName` properties to the server side. The server returns back a stream of `SystemProperty`[hotspot=SystemProperty file=0] messages.

To compile the `.proto` file, the `pom.xml` Maven configuration file needs the  `grpc`[hotspot=grpc file=1] and `grpc-stub`[hotspot=grpc-stub file=1] dependencies. To install the correct version of Protobuf compiler automatically, the `os-maven-plugin`[hotspot=protobufmavenplugin file=1] extension is required.
