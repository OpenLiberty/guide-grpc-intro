...
The endpoint creates a [hotspot=createClient3 file=2]`SystemServiceClient` and uses the client to collect system properties that start with `user.`, shuts down the channel, and returns the values from the `system` service response.
