...
The endpoint creates a [hotspot=createChannel file=0]`channel` to the `system` service and a [hotspot=createClient file=0]`client` by using the `SystemServiceGrpc.newStub()` API. Then, call the [hotspot=getPropertiesServer file=0]`getPropertiesServer()` method with an implementation of the `StreamObserver` interface. The [hotspot=onNext file=0]`onNext()` method receives messages streaming from the server individually. After all properties are received, shutdown the [hotspot=closeConnection file=0]`channel`.  Because the rpc call is asynchronous, use the [hotspot=countDownLatch file=0]`CountDownLatch` to synchronize the streaming flow.
